...
def check_directions(tile, trail, visited):
    possibilities = []
    char = trail[tile[0]][tile[1]]

    if char == "<":
        next = (tile[0],tile[1]-1) #left
    elif char == ">":
        next = (tile[0],tile[1]+1) #right
    elif char == "v":
        next = (tile[0]+1,tile[1]) #down
    else:
...
def solve1(input):
    ...
    while len(q)>0:
        ...
        total, other_routes = check_route(next, trail,e)
